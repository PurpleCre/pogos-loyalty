...
export const useAchievements = () => {
  ...
  const { data: achievements, isLoading: achievementsLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!isOnline) {
        const cached = await getOfflineData('achievements');
        if (cached) {
          return cached;
        }
      }
  ...
  const { data: userAchievements, isLoading: userAchievementsLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!isOnline) {
        const cached = await getOfflineData(`userAchievements_${user.id}`);
        if (cached) {
          return cached;
        }
      }
  ...
  const getAchievementStatus = (achievementId: string) => {
    return userAchievements?.some((ua: any) => ua.achievement_id === achievementId) || false;
  };
