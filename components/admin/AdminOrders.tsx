...
import { Clock, CheckCircle2, ChevronRight, Package, Store } from 'lucide-react-native';

interface AdminOrdersProps {
  stores: { id: string; name: string }[];
}

export function AdminOrders({ stores }: AdminOrdersProps) {
  ...
  const getStoreName = (storeId?: string) => {
    if (!storeId) return 'Online / Unknown Store';
    const store = stores.find(s => s.id === storeId);
    return store ? store.name : 'Unknown Store';
  };

  const groupedOrders = activeOrders.reduce((acc, order) => {
    // Assuming order has store_id, if not we fallback
    const sId = (order as any).store_id || 'unassigned';
    if (!acc[sId]) acc[sId] = [];
    acc[sId].push(order);
    return acc;
  ...
  return (
    <View className="gap-6">
      {Object.entries(groupedOrders).map(([storeId, storeOrders]) => (
        <View key={storeId} className="gap-3">
          <View className="flex-row items-center gap-2 mb-1 pl-1">
            <Store size={18} color="#4b5563" />
            <Text className="font-bold text-gray-700 text-base">{getStoreName(storeId === 'unassigned' ? undefined : storeId)}</Text>
            <View className="bg-gray-200 px-2 py-0.5 rounded-full ml-auto">
              <Text className="text-xs font-bold text-gray-600">{storeOrders.length}</Text>
            </View>
          </View>
